...
from paprika.executors.ScheduledEventWatcher import ScheduledEventWatcher
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description="paprika-scheduler agent")
    ...
    f = Finder.open(properties, 'paprika_scheduler.json')
    settings = json.load(f)

    abort = Abort()
    threads = Threads(abort)
    t = Runner.run(WatchWorker(ScheduledEventWatcher(), settings, threads, abort))
