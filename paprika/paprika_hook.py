...
from paprika.executors.HookWatcher import HookWatcher
...
def main(args=None):
    ...
    f = Finder.open(properties, 'paprika_hook.json')
    settings = json.load(f)

    abort = Abort()
    threads = Threads(abort)
    t = Runner.run(WatchWorker(HookWatcher(), settings, threads, abort))
